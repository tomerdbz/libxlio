...
const mce_sys_var &safe_mce_sys()
{
    return mce_sys_var::safe_instance();
}

mce_sys_var &mce_sys_var::safe_instance_non_const()
{
    mce_sys_var &instance = mce_sys_var::instance();
    if (instance.get_frozen()) {
        throw std::runtime_error(
            "Calling mce_sys_var::safe_instance_non_const() is forbidden, mce_sys_var is frozen");
    }
    return instance;
}
...
void mce_sys_var::freeze() const
{
    m_frozen = true;
}

bool mce_sys_var::unfreeze() const
{
...
void mce_sys_var::get_env_params()
{
    ...
    // Used only in env-vars configuration flow.
    // When using the config-file flow, enable_striding_rq is set directly.
